...
<div class="comic-card group">
    <a href="{{ route('comic.show', $comic->slug) }}" class="block">
        <div class="relative overflow-hidden rounded-2xl bg-dark-900/70 border border-white/5">
            <img src="{{ $comic->cover_url }}"
                 alt="{{ $comic->title }}"
                 class="comic-cover w-full aspect-[2/3] object-cover"
                 loading="lazy">
            ...
            <!-- Type Badge -->
            <span class="absolute top-3 left-3 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg"
                  style="background: {{ $comic->type->color ?? '#06b6d4' }}">
                {{ $comic->type->name ?? 'Comic' }}
            </span>

            <!-- Status/Hot Badge -->
            @if($comic->is_hot)
                <span class="absolute top-3 right-3 px-2.5 py-1 bg-secondary-500 text-[10px] font-black rounded-full shadow-lg badge-new flex items-center gap-1">
            ...
            <!-- Rating Overlay -->
            <div class="absolute bottom-3 left-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <div class="mt-3 px-1">
        ...
        <div class="mt-2 space-y-1.5">
            ...
            <a href="{{ route('chapter.read', [$comic->slug, $ch->slug]) }}"
               class="flex items-center justify-between text-[11px] px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 transition group/ch border border-white/5">
