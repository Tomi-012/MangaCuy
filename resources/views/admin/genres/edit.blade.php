...
<div class="max-w-3xl">
    <div class="bg-dark-900/50 border border-dark-800 rounded-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-dark-800 flex justify-between items-center bg-dark-900/80">
            <h3 class="font-bold text-lg">Edit Data Genre</h3>
            <a href="{{ route('admin.genres.index') }}" class="text-gray-400 hover:text-white transition text-sm">Kembali</a>
        </div>
        
        <form action="{{ route('admin.genres.update', $genre) }}" method="POST" class="p-6">
            @csrf @method('PUT')
            ...
            <div class="mb-5">
                ...
                <div class="flex items-center gap-3">
                    <input type="color" id="colorPicker" value="{{ old('color', $genre->color ?? '#6366f1') }}" class="h-11 w-11 rounded-xl cursor-pointer bg-dark-800 border border-dark-700 p-1"
                           onchange="document.getElementById('color').value = this.value">
                    <input type="text" id="color" name="color" value="{{ old('color', $genre->color ?? '#6366f1') }}"
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full bg-dark-800 border {{ $errors->has('description') ? 'border-red-500' : 'border-dark-700' }} rounded-xl px-4 py-3 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500/50 transition">{{ old('description', $genre->description) }}</textarea>
            </div>
            
            <div class="flex items-center gap-3">
                <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition shadow-lg shadow-blue-600/30">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
