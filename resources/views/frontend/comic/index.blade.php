...
<div class="page-container page-section">
    ...
    <div class="section-header">
        ...
        <div class="flex flex-wrap gap-2">
            <div class="relative">
                <select onchange="updateFilter('sort', this.value)" class="select pr-10 bg-white/5">
            ...
            <div class="relative">
                <select onchange="updateFilter('type', this.value)" class="select pr-10 bg-white/5">
            ...
            <div class="relative">
                <select onchange="updateFilter('status', this.value)" class="select pr-10 bg-white/5">
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        <a href="{{ route('comics.index', request()->except('genre')) }}"
           class="chip {{ !request('genre') ? 'bg-primary-600 text-white border-transparent' : 'bg-white/5 text-gray-400 hover:bg-white/10' }}">
            Semua
        </a>
        @foreach($genres as $genre)
        <a href="{{ route('comics.index', array_merge(request()->all(), ['genre' => $genre->slug])) }}"
           class="chip {{ request('genre') == $genre->slug ? 'bg-primary-600 text-white border-transparent' : 'bg-white/5 text-gray-400 hover:bg-white/10' }}">
            {{ $genre->name }}
        </a>
        @endforeach
    </div>
